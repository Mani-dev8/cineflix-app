...
import Card from './CategoryListCard'
...
const ContainerSpecial = ({endPoint,title}:Props) => {
  const [topRated, setTopRated] = useState<TopMovie[]>([])
  const navigation = useNavigation()
  async function topTvShow() {
    const response = await fetch(
      `${baseUrl}${endPoint}?api_key=${api_key}`,
    )
    const movies = await response.json()
    const storedWatchList = await AsyncStorage.getItem('watchListTv');
       ...
       const updatedMovies = topRated.map(movie => {
         if (movie.id === id) {
           if (movie.watchlisted) {
             ...
             // Add to watch list
             addToWatchList(movie.id, movie.name, movie.poster_path);
  ...
  const addToWatchList = async (movieId: number, movieName: string,posterPath:string) => {
    try {
      ...
      await AsyncStorage.setItem('watchListTv', JSON.stringify(updatedWatchList));
    } catch (error) {
